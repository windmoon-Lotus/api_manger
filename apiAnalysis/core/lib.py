...
from apiAnalysis.core.identify.direct import workspace_auth_conf, deal_with_direct
from apiAnalysis.core.identify.sso import workspace_sso_conf, deal_with_sso
from apiAnalysis.model.model import *
from apiAnalysis.model.exception import *
from apiAnalysis.common.util import deal_request, gen_banner
...
def monogo_deal_scan(name, header: HeaderModel, body: BodyModel, ws: Workspace, roles, row ) -> PacketRecord:


    packet_record = PacketRecord(ws_id=ws.id, username=name, raw_packet=ObjectIdField(row["_id"]))
    #print()
    try:
