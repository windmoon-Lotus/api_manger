...
class MitmproxyFlowWrapper:
    ...
    def get_url(self) -> str:
        if self.flow.request.url.endswith(
                ("js", "css", "ttf", "jpg", "svg", "gif", "jpeg", "png", "pdf", "rar", "svg", "ico"
                 , "woff", "woff2", "otf", "zip", "docx")):
            return "None"
                ...
                for f in freader.stream():
                    ...
                    if isinstance(f, http.HTTPFlow):
                        if f.response is None:
                            logger.debug("flow without response: %s", f.request.url)
                            continue
                        yield MitmproxyFlowWrapper(f)
            except FlowReadException as e:
                logger.warning("Flow file corrupted: %s", e)
